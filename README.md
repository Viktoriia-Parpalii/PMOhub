...
## Групування та порядок завдань скоупу

Завдання квартальної картки можна об’єднувати в локальні групи, перейменовувати й розформовувати, а порядок груп і завдань змінювати стрілками у модалці картки. Порядок зберігається лише після натискання «Зберегти».

Нумерація формується автоматично: групи й окремі завдання мають номери верхнього рівня (1., 2.), а завдання всередині групи — ієрархічні (1.1, 1.2). Заголовок групи отримує колір статусу лише тоді, коли всі її завдання мають однаковий статус; інакше він нейтральний.

Міграція 20260918120000_add_scope_groups_and_ordering є недеструктивною: наявні завдання залишаються без груп, отримують стабільний sort_order, а тексти, статуси, ваги, виконавці та lineage не змінюються. Перед застосуванням у середовищі з реальними даними обов’язково створіть резервну копію БД.

Excel зберігає ієрархію та нумерацію скоупу. AI JSON, як і раніше, не містить назв груп або текстів завдань — лише агрегати.
