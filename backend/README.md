...
## Групи та порядок скоупу

Модель scope_groups належить одній квартальній картці. scope_items.scope_group_id є nullable, а scope_items.sort_order задає canonical-порядок усіх завдань картки. Група переноситься між кварталами за стабільним lineage_id.

Контракти create/update квартальної картки приймають scope_groups[] та group_id для кожного елемента scope[]. Порядок масиву scope[] є canonical-порядком.

Backend перевіряє належність груп картці, унікальність назв та неперервність завдань кожної групи. Порожні групи не зберігаються. Під час копіювання або перенесення завдання його група відтворюється в цільовій картці за lineage_id, після чого порядок обох карток нормалізується.

Перед застосуванням 20260918120000_add_scope_groups_and_ordering до БД із даними зробіть backup. Міграція перевіряє кількість і ID завдань та кількість зв’язків із виконавцями; при невідповідності транзакція відкочується.

Після змін DTO виконайте npm run openapi, а потім npm run api:generate у frontend.
