...
import {
  ...
  InitiativeSizeDef, InitiativeYearSnapshot, Manager, MutationResult, OperationalTask,
...
import {
  ...
  initialPriorities, initialProjects, initialRolePermissions, initialTaskWeights,
...
import {
  ...
  moveCard as moveCardRecord, moveChecklistItem as moveChecklistItemRecord,
...
const initialState: AppDataState = {
  departments: initialDepartments, priorities: initialPriorities, taskWeights: initialTaskWeights,
  initiativeSizes: initialInitiativeSizes, managers: initialManagers,
  projects: reconcileBacklogYears(initialProjects), tasks: reconcileBacklogYears(initialTasks),
...
export const validateExportData = (input: unknown): MutationResult<FullExportData> => {
  ...
  if (data.version !== '3.0') return fail('Підтримується лише формат PMO Hub 3.0');
  const allowedKeys = new Set<keyof FullExportData>([
    'version', 'exportedAt', 'exportedBy', 'departments', 'priorities', 'taskWeights',
  ...
  const requiredArrays: Array<keyof FullExportData> = [
    'departments', 'priorities', 'taskWeights', 'initiativeSizes', 'managers',
  ...
  const invalidCard = all.find(record => !record.is_backlog && validateChecklistCapacity(record.checklist, normalized.taskWeights).length > 0);
...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addInitiative = <T extends Initiative>(kind: InitiativeKind, raw: T): MutationResult => {
    const initiative = sanitizeInitiative(raw); const records = recordsFor(kind); const permission = getPermissions(state.currentUser, state.rolePermissions);
  ...
  const updateInitiative = <T extends Initiative>(kind: InitiativeKind, id: string, patch: Partial<T>): MutationResult => {
    ...
    const updated = sanitizeInitiative({ ...record, ...patch } as T); const errors = validateChecklistCapacity(updated.checklist, state.taskWeights); if (errors.length) return fail(errors.join('\n'));
  ...
  const savePassport = (command: SavePassportCommand): MutationResult<{ snapshots: number; cards: number }> => {
    ...
    const next = records.map(record => {
      ...
      if (sourceCard && record.id === sourceCard.id) return { ...record, ...patch, ...(command.sourceCardPatch ?? {}), history: [event, ...(record.history ?? [])] };
  ...
  const createBacklogWithCards = (kind: InitiativeKind, raw: Project | OperationalTask, quarters: Quarter[], initialScope: Project['checklist'] = []): MutationResult => {
    ...
    const cards = uniqueQuarters.map(quarter => sanitizeInitiative({
  ...
  const priorityOps = listOps<PriorityDef>('priorities'); const departmentOps = listOps<Department>('departments'); const managerOps = listOps<Manager>('managers'); const customFieldOps = listOps<CustomFieldDef>('customFields');
  const addTaskWeight = (item: TaskWeightDef): MutationResult => item.weight < 0 ? fail('Вага не може бути від’ємною') : adminMutation('Вагу додано', current => ({ ...current, taskWeights: [...current.taskWeights, item] }));
  const updateTaskWeight = (id: string, patch: Partial<TaskWeightDef>): MutationResult => { const item = state.taskWeights.find(weight => weight.id === id); if (!item) return fail('Вагу не знайдено'); if ((patch.weight ?? item.weight) < 0) return fail('Вага не може бути від’ємною'); return adminMutation('Вагу оновлено', current => ({ ...current, taskWeights: current.taskWeights.map(weight => weight.id === id ? { ...weight, ...patch } : weight) })); };
  const deleteTaskWeight = (id: string): MutationResult => { if ([...state.projects, ...state.tasks].some(record => record.checklist.some(item => item.weightId === id))) return fail('Вага використовується. Деактивуйте її замість видалення'); return adminMutation('Вагу видалено', current => ({ ...current, taskWeights: current.taskWeights.filter(item => item.id !== id) })); };
  ...
  const getFullDataSnapshot = (): FullExportData => ({
    version: '3.0', exportedAt: new Date().toISOString(), exportedBy: state.currentUser ? { id: state.currentUser.id, name: state.currentUser.name, email: state.currentUser.email, role: state.currentUser.role } : undefined,
    departments: state.departments, priorities: state.priorities, taskWeights: state.taskWeights, initiativeSizes: state.initiativeSizes,
  ...
  const importFullData = (raw: FullExportData, mode: 'replace' | 'merge') => {
    ...
    apply(current => { const select = <T extends { id: string }>(existing: T[], incoming: T[]) => mode === 'replace' ? incoming : mergeById(existing, incoming); return {
      ...current, departments: select(current.departments, data.departments), priorities: select(current.priorities, data.priorities), taskWeights: select(current.taskWeights, data.taskWeights), initiativeSizes: select(current.initiativeSizes, data.initiativeSizes), managers: select(current.managers, data.managers),
  ...
  const value: AppContextType = {
    ...
    moveInitiative, moveCard, moveScopeItem, savePassport, createBacklogSnapshot, createBacklogSnapshots, createBacklogWithCards, updatePreparationStage,
    addPriority: priorityOps.add, updatePriority: priorityOps.update, deletePriority: priorityOps.remove,
    addTaskWeight, updateTaskWeight, deleteTaskWeight, addInitiativeSize, updateInitiativeSize, deleteInitiativeSize,
    addDepartment: departmentOps.add, updateDepartment: departmentOps.update, deleteDepartment: departmentOps.remove,
    addManager: managerOps.add, updateManager: managerOps.update, deleteManager: managerOps.remove,
