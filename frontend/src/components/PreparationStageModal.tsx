  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
    <div className="w-full max-w-xl rounded-2xl border border-slate-200 bg-white shadow-2xl">
      ...
      <div className="space-y-5 p-6">{error && <div className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm font-bold text-rose-700">{error}</div>}
        <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-bold text-slate-700">Менеджер<select value={managerId} onChange={event => setManagerId(event.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-300 px-4 py-3 font-normal outline-none focus:border-indigo-500"><option value="">Не обрано</option>{managers.filter(manager => manager.is_active !== false || manager.id === managerId).map(manager => <option key={manager.id} value={manager.id}>{manager.name}</option>)}</select></label><label className="text-sm font-bold text-slate-700">Пріоритет<select value={priority} onChange={event => setPriority(event.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-300 px-4 py-3 font-normal outline-none focus:border-indigo-500"><option value="">Не обрано</option>{priorities.filter(value => value.is_active !== false || value.id === priority).map(value => <option key={value.id} value={value.id}>{value.name}</option>)}</select></label></div>
        <div><p className="mb-2 text-sm font-bold text-slate-700">Залучені підрозділи</p><div className="flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3">{departments.filter(department => department.is_active !== false || departmentIds.includes(department.id)).map(department => <button type="button" key={department.id} onClick={() => toggle(department.id)} className={`rounded-lg border px-3 py-2 text-xs font-bold transition ${departmentIds.includes(department.id) ? 'border-amber-400 bg-amber-500 text-white' : 'border-slate-200 bg-white text-slate-600 hover:border-indigo-300'}`}>{departmentIds.includes(department.id) ? '✓ ' : ''}{department.name}</button>)}</div></div>
