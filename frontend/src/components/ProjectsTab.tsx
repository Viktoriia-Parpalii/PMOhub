...
export const ProjectsTab = () => {
  const { projects, updateProject, currentUser, customFields, departments, managers, priorities, deleteProject, rolePermissions, savePassport, createBacklogWithCards } = useAppContext();
  ...
  if (currentUser?.role === 'USER' && currentUser.departmentId) {
    portfolioProjects = portfolioProjects.filter(p => (p.implementer_dept_ids || []).includes(currentUser.departmentId!) || (p.cross_functional_dept_ids || []).includes(currentUser.departmentId!));
  }
          ...
          <table className="min-w-max divide-y divide-slate-200 text-sm w-full table-fixed">
            ...
            <tbody className="divide-y divide-slate-100">
              {portfolioProjects.map(p => {
                ...
                return (
                <tr key={p.id} className={`${getRowBgClass(status)} transition-colors border-b border-slate-100/80`}>
                    ...
                    <div className="flex flex-col items-start gap-2">
                      ...
                      {canEdit && (
                        <div className="flex items-center gap-1.5 rounded-lg border border-slate-200/80 bg-white/90 p-1 backdrop-blur shadow-2xs" aria-label="Обрати статус ініціативи">
                          <button 
                            onClick={(e) => { e.stopPropagation(); updateProject(p.id, { health_status: 'DEFAULT' as any }); }}
                          ...
                          />
                          <button 
                            onClick={(e) => { e.stopPropagation(); updateProject(p.id, { health_status: 'GREEN' as any }); }}
                          ...
                          />
                          <button 
                            onClick={(e) => { e.stopPropagation(); updateProject(p.id, { health_status: 'YELLOW' as any }); }}
                          ...
                          />
                          <button 
                            onClick={(e) => { e.stopPropagation(); updateProject(p.id, { health_status: 'RED' as any }); }}
                  ...
                  <td className="min-w-[10rem] px-4 py-3 text-center whitespace-nowrap">
                    <span className={`inline-flex w-32 justify-center truncate rounded-full border px-2.5 py-1 text-xs font-bold ${getPriorityBadgeClass(priority?.id ?? p.priority)}`} title={priority?.name ?? p.priority ?? 'Не обрано'}>
