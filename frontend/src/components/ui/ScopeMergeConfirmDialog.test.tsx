...
describe('ScopeMergeConfirmDialog', () => {
  it('shows merge consequences and exposes cancel/confirm actions', () => {
    ...
    expect(screen.getByText('Підтвердіть перенесення завдання.')).toBeInTheDocument();
    expect(screen.getByText(/I-1/)).toBeInTheDocument();
    expect(screen.getByText(/картку буде видалено з кварталу/)).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: 'Скасувати' }));
    fireEvent.click(screen.getByRole('button', { name: 'Об’єднати скоуп' }));
