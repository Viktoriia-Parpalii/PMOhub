...
export const normalizeHealthStatus = (
  status: HealthStatus,
): CanonicalHealthStatus => (status === "GRAY" ? "DEFAULT" : status);

export const healthCounts = (cards: AnalyticsCard[]) =>
  cards.reduce<Record<CanonicalHealthStatus, number>>(
    (counts, card) => {
      const status = normalizeHealthStatus(
        card.health_status_code ?? card.health_status,
      );
...
export const averageInitiativeDuration = (cards: AnalyticsCard[]): number => {
  ...
  cards.forEach((card) => {
    ...
    quarterCountByInitiative.set(
      key,
      (quarterCountByInitiative.get(key) ?? 0) + 1,
    );
  });

  if (!quarterCountByInitiative.size) return 0;
  const totalQuarters = Array.from(quarterCountByInitiative.values()).reduce(
    (sum, count) => sum + count,
    0,
  );
...
export const scopeStatusCounts = (cards: AnalyticsCard[]) =>
  cards
    .flatMap((card) => card.checklist)
    .reduce<Record<CanonicalHealthStatus, number>>(
      (counts, item) => {
...
export const capacityByQuarter = (
  ...
  (["Q1", "Q2", "Q3", "Q4"] as const).map((quarter) => ({
    quarter,
    loads: calculateDepartmentLoads(
