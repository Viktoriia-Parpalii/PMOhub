...
const legacy: Record<string, { label: string; color: string }> = {
  DEFAULT: { label: "Без статусу", color: "#94a3b8" },
  GRAY: { label: "Без статусу", color: "#94a3b8" },
  GREEN: { label: "Виконано", color: "#10b981" },
  YELLOW: { label: "В процесі", color: "#f59e0b" },
  RED: { label: "На паузі / блоковано", color: "#f43f5e" },
};
...
export const getInitiativeStatusStyle = (
  status: HealthStatus,
  statuses: InitiativeStatusDef[],
): React.CSSProperties => {
...
export const getHealthStatusPresentation = (
  status: HealthStatus = "DEFAULT",
...
export const getHealthLabel = (status: HealthStatus = "DEFAULT"): string =>
  getInitiativeStatus(status).name;
export const getHealthColors = (
  status: HealthStatus = "DEFAULT",
): HealthStatusPresentation["colors"] =>
  getHealthStatusPresentation(status).colors;
