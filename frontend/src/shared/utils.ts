...
export const getComputedTotalWeight = (
  checklist: any[] | undefined,
  taskWeights: TaskWeightDef[],
  targetYear?: number,
  targetQuarter?: import("./types").Quarter,
): number => {
  void targetYear;
  void targetQuarter;
...
export const getLatestPriorState = <
  T extends { year: number; quarter: string },
>(
  backlogItem: T,
  targetYear: number,
...
export const getNextPeriod = (
  y: number,
  q: string,
): { year: number; quarter: import("./types").Quarter } => {
...
export const getFutureQuarters = (
  selectedYear: number,
  itemYear: number,
  itemQuarter: string,
): import("./types").Quarter[] => {
...
export const getValidQuarters = (
  selectedYear: number,
  itemYear?: number,
  itemQuarter?: import("./types").Quarter,
): import("./types").Quarter[] => {
  ...
  if (selectedYear === curYear) {
    ...
    return allQuarters.filter(
      (q) => qToNum(q) >= curNum || (itemYear === curYear && itemQuarter === q),
    );
  }
