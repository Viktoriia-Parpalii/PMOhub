...
```
## Керування скоупом квартальної картки

У модалці квартальної картки для кожного завдання доступний вибір «Без групи», наявної групи або створення нової. Заголовок групи можна перейменувати, групу — перемістити стрілками чи розформувати. Стрілки завдання змінюють порядок у межах його групи, а для завдань без групи — порядок серед верхньорівневих блоків.

Нумерація розраховується спільним helper: група або окреме завдання має номер 1., 2.; завдання групи — 1.1, 1.2.

Однакова структура використовується у grid-картках, таблиці, модалці й архівному readonly-перегляді. Заголовок групи кольоровий лише для однорідного статусу всіх завдань.

Групування й порядок є локальним станом модалки до збереження. Чинні заборони для виконаних завдань на видалення, копіювання та перенесення залишаються активними.
